...
async fn watch_v(Extension(state): Extension<Arc<State>>,Query(params): Query<HashMap<String, String>>){
    todo!()
}

/// Handler for the paths /w/:id, /v/:id,/e/:id, /shorts/:id and /watch/:id it redirects to the /watch/v=id path
async fn redirect(Extension(state): Extension<Arc<State>>,Path(id): Path<String>) -> Redirect{
    todo!()
}
...
async fn clip(Extension(state): Extension<Arc<State>>,Path(id): Path<String>) -> Redirect{
    todo!()
}
